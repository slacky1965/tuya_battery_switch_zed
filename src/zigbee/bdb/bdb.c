...
_CODE_BDB_ static void bdb_SimpleDescResp(void *arg)
{
    ...
    APP_DEBUG(DEBUG_BDB_EN, "bdb_SimpleDescResp, flag: %d, ep: %d, status: %d\r\n", findbind->find_bind_flag, simpDescResp->simple_descriptor.endpoint, simpDescResp->status);
        ...
        if (inClusterNum) {
            ...
            if (g_bdbCtx.matchClusterList) {
                ...
                if (g_bdbCtx.matchClusterNum) {
                    ...
                    if (findbind->find_bind_flag) g_bdbCtx.bindDstInfo.endpoint = findbind->find_bind_dst_ep;
...
_CODE_BDB_ static void bdb_commissioningFindBindSimpleDescReq(void)
{
    ...
    if (BDB_STATE_GET() == BDB_STATE_COMMISSIONING_FINDORBIND &&
        ...
        if (findbind->find_bind_flag) {
            ...
            req.endpoint = findbind->find_bind_dst_ep;
        } else {
    ...
    if (g_bdbAttrs.commissioningMode.findOrBind) {
        if (g_bdbAttrs.nodeIsOnANetwork) {
            ...
            if (g_bdbCtx.role == BDB_COMMISSIONING_ROLE_TARGET) {
                ...
                if (findbind->find_bind_flag) {
                    ep = findbind->find_bind_src_ep;
                    APP_DEBUG(DEBUG_BDB_EN, "zcl_identify_identifyQueryCmd with ep: %d form app\r\n", ep);
                } else {
                    ep = g_bdbCtx.simpleDesc->endpoint;
                    APP_DEBUG(DEBUG_BDB_EN, "zcl_identify_identifyQueryCmd with ep: %d form simpleDesc\r\n", ep);
                }
