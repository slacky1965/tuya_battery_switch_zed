...
void app_gpio_init(int anaRes_init_en) {
    ...
    if (*(uint32_t*)(DEVICE_MODEL_CFG_ADDR) == ID_DEVICE_MODEL_CFG) {
        uint8_t model = *(uint8_t*)(DEVICE_MODEL_CFG_ADDR+4);
    ...
    if (device_model == DEVICE_MODEL_1) {
        ...
        //PC group
        //ie
        analog_write(areg_gpio_pc_ie,   (PC0_INPUT_ENABLE<<0)   | (PC1_INPUT_ENABLE<<1) | (1<<2) | (PC3_INPUT_ENABLE<<3) |
                                        (PC4_INPUT_ENABLE<<4)   | (PC5_INPUT_ENABLE<<5) | (PC6_INPUT_ENABLE<<6) | (PC7_INPUT_ENABLE<<7) );

        //oen
        reg_gpio_pc_oen =
            ((PC0_OUTPUT_ENABLE?0:1)<<0)    | ((PC1_OUTPUT_ENABLE?0:1)<<1) | (0<<2)   | ((PC3_OUTPUT_ENABLE?0:1)<<3) |
        ...
        reg_gpio_pc_gpio =
            (PC0_FUNC==AS_GPIO ? BIT(0):0)  | (PC1_FUNC==AS_GPIO ? BIT(1):0)| (BIT(2))| (PC3_FUNC==AS_GPIO ? BIT(3):0) |
