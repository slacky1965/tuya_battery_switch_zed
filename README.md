...
> [!WARNING]
> Внимание!!! На момент выхода прошивки конвертор не был добавлен в репозиторий `zigbee-herdsman-converters`, поэтому в этой версии `zigbee2mqtt` [внешний конвертор](zigbee2mqtt/TS0044-z-SlD.js) нужно положить в директорию `external_converters` и перегрузить `zigbee2mqtt`. Начиная с `zigbee2mqtt` версии 2.9.2 внешний конвертор не нужен.

Если вы ещё не подключили выключатель к `zigbee2mqtt`, то нужно разрешить спаривание в `zigbee2mqtt` и на 10 секунд зажать кнопку выключателя. После этого светодиод выключателя начнёт мигать, а спустя несколько секунд вы увидите в `zigbee2mqtt` новое устройства.

Для перепрошивки скачиваем из репозитория нужный файл обновления (какой именно смотрите в таблице вверху). Заходим в устройство. И справа видим в `Firmware version` значок облака. Нам сюда.
...
Чтобы понять, пошло обновление или нет, смотрим в `Recent activity` - там будет отображаться оставшееся время в секундах и сколько загрузили в процентах. Плюс, в общем списке всех устройств (а не на странице данного выключателя) на изображении выключателя должен появиться вращающийся кружок со стрелками.

> [!WARNING]
> Во время обновления кнопки на выключателе нажимать НЕ нужно!
> ЗАРЯД батареек должен быть не менее 50%! Возможно, чуть меньше (надо проверять). Но если в районе этих значений обновление не стартует, попробуйте заменить батарейку.
...
Привычный репортинг есть только у батарейки и все равно его пришлось немного модифицировать (в угоду сбережения заряда батарейки). Считывание показаний батарейки и отправка репорта зависят от настройки `Max rep interval`.
...
Также можно забиндить какое-нибудь устройство, которое поддерживает технологию `Find and bind`. Чтобы ввести выключатель в режим поиска нужно 4 раза быстро нажать нужную кнопку и затем в течение 3 секунд нажать и удерживать эту же кнопку.
...
- 1.0.01
	- Добавлена кнопка `_TZ3000_an5rjiwd`.
