...
| TS0042-M003-SlD | TS0042 | [TS0042](https://www.zigbee2mqtt.io/devices/TS0042.html) | _TZ3000_h68nee3e | OTA file | :bookmark_tabs: | 
...
- `Switch type (Endpoint: X)` - выбор действия при нажатии на кнопку.
	...
	- `brightness_level_up` (**для 1-но кнопочных версий `brightness_level`**) - устанавливает режим работы кнопки для увеличения яркости (**для 1-но кнопочных версий устанавливает режим работы кнопки для попеременного увеличения и уменьшения яркости**).
		- Одно нажатие - посылает команду `On` на устройство (**для 1-но кнопочных версий посылает команду `toggle`**).
		...
		- Удержание - посылает команду `Move`. Устройство должно плавно увеличивать яркость, пока последняя не станет максимальной. Увеличение продолжается, пока удерживается кнопка. Если устройство было выключено, оно включается. **Для 1-но кнопочных версий первое удержание приводит в увеличению яркости, следующее удержание приводит к уменьшению яркости и так по кругу**.
